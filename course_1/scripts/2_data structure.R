...
str(sex_new)
##ordinal
star<-c('heart','crown','diamond','heart','diamond')
factor(star,ordered=T,levels=c('heart','diamond',
                               'crown'))
...
patientdata <- data.frame(patientID,gender,age,diabetes,
                          status)
patientdata
patientdata$gender<-with(patientdata,factor(gender,levels=c(1,2),labels=c('male','female')))
str(patientdata)
summary(patientdata)
...
#    - foreget to load packages needed.
